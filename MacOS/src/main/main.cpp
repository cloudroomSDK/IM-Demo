...
void initApp(const QString &appFileName)
{
    srand(QDateTime::currentMSecsSinceEpoch());
	QFileInfo qfinfo(appFileName);
...
{
	QString appFileName(QString::fromLocal8Bit(argv[0]));
	initApp(appFileName);
	...
	g_sdkMain = CRVideoSDKMain::create(QDir::currentPath().toUtf8().constData(), pDispatch, "");
	if (g_sdkMain == NULL)
	{
		QMessageBox::information(NULL, QObject::tr("提示"), QObject::tr("创建sdk对象失败!"));
		return -1;
	}
	LoginDlg *w = new LoginDlg;
	w->show();
