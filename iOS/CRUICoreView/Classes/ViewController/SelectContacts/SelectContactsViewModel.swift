...
class SelectContactsViewModel {
    ...
    func getMyFriendList() {
        IMController.shared.getFriendList { [weak self] users in
            guard let `self` = self else { return }
            self.contacts = users.map{ContactInfo(ID: $0.userID, name: $0.nickname, faceURL: $0.faceURL, type: .friends)}
    ...
    func getGrpMemberList(groupID: String) {
        IMController.shared.getGrpMemberList(groupId: groupID, filter: .all, offset: 0, count: 1000) { [weak self] (members: [GroupMemberInfo]) in
            guard let `self` = self else { return }
    ...
    func search(keyword: String) {
        ...
        DispatchQueue.global().async {
            ...
            group.enter()
            IMController.shared.searchGrps(param: param) {[weak self] result in
