...
public class SelectContactsViewController: UIViewController {
    ...
    private lazy var _tableView: UITableView = {
        ...
        config.indexItemSelectedBackgroundColor = UIColor.c0584FE
    ...
    private func bindData() {
        ...
        _viewModel.searchResult.subscribe(onNext: { [weak self] r in
            ...
            self?.resultVC.updateSearchResults(text: self?.navigationItem.searchController?.searchBar.text?.trimmingCharacters(in: .whitespacesAndNewlines).lowercased())
        }).disposed(by: _disposeBag)
    ...
    private func loadData() {
        ...
        if contacTypes.contains(.members) {
            ...
            _viewModel.getGrpMemberList(groupID: sourceID!)
        }
    }
