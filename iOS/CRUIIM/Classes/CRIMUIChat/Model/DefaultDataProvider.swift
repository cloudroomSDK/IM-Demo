...
protocol DataProvider {
    ...
    func getGrpInfo(groupInfoHandler: @escaping (GroupInfo) -> Void)
}
...
final class DefaultDataProvider: DataProvider {
    ...
    init(conversation: ConversationInfo, anchorID: String? = nil) {
        ...
        self.receiverId = conversation.conversationType == .c1v1 ? conversation.userID! : conversation.groupID!
    ...
    func loadInitialMessages(completion: @escaping ([MessageInfo]) -> Void) {
        if anchorID != nil {
            ...
            group.enter()
            getHistoryMsgList { ms in
                r.append(contentsOf: ms)
                group.leave()
            }
            group.enter()
            getHistoryMsgList(reverse: true) { ms in
                r.append(contentsOf: ms)
                group.leave()
            }
            ...
            getHistoryMsgList(completion: completion)
        }
    }
    ...
    func getGrpInfo(groupInfoHandler: @escaping (GroupInfo) -> Void) {
        IMController.shared.getGrpInfo(groupIds: [receiverId]) { [weak self] (infos: [GroupInfo]) in
    ...
    private func getHistoryMsgList(reverse: Bool = false, completion: @escaping ([MessageInfo]) -> Void) {

        if reverse {
            IMController.shared.getHistoryMsgListReverse(conversationID: conversation.conversationID,
            ...
            IMController.shared.getHistoryMsgList(conversationID: conversation.conversationID,
    ...
    private func getSelfInfoInGroup() {
        IMController.shared.getGrpMembersInfo(groupId: receiverId, uids: [IMController.shared.userID]) { [weak self] ms in
