...
class MemberListViewModel {
    ...
    func getOwnerAndAdmin() {
        IMController.shared.getGrpMemberList(groupId: groupInfo.groupID, filter: .superAndAdmin, offset: 0, count: limit) { [weak self] infos in
            self?.ownerAndAdminRelay.accept(infos)
        }
    }

    func getMoreMembers(completion: ((Bool) -> Void)?) {
        IMController.shared.getGrpMemberList(groupId: groupInfo.groupID, filter: .member, offset: offset, count: limit) { [weak self] (members: [GroupMemberInfo]) in
