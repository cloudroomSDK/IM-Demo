...
class GroupListViewModel {
    ...
    func getMyGroups() {
        IMController.shared.getJoinedGrpList { [weak self] (groups: [GroupInfo]) in
            let groups: [GroupInfo] = groups ?? []
            var createGroups: [GroupInfo] = []
            var joinedGroups: [GroupInfo] = []
            for group in groups {
                if group.creatorUserID == IMController.shared.userID {
    ...
    func getGrpMemberList(groupID: String) {
        IMController.shared.getGrpMemberList(groupId: groupID, offset: 0, count: 1000) { (members: [GroupMemberInfo]) in
