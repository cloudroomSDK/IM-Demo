...
class UserProfileViewModel {
    ...
    func getUserOrMemberInfo() {
        if let groupId = groupId, groupId.isEmpty == false {
            IMController.shared.getGrpMembersInfo(groupId: groupId, uids: [userId]) { [weak self] (members: [GroupMemberInfo]) in
                self?.memberInfoRelay.onNext(members.first)
            }
        }
        
        if let handler = CRIMApi.queryUsersInfoWithCompletionHandler {
    ...
    func sendCard(cardUser: UserInfo, to recvID: String, conversationType: ConversationType = .c1v1) {
        let card = CardElem(userID: cardUser.userID, nickname: cardUser.nickname!, faceURL: cardUser.faceURL)
        
        IMController.shared.sendCardMsg(card: card,
                                            to: recvID,
