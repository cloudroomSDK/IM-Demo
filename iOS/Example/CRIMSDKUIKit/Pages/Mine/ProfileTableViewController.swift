...
class ProfileTableViewController: CRUIIM.ProfileTableViewController {
    ...
    override func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch rowType {
        case .avatar:
            presentActionSheet(action1Title: "相册".innerLocalized(), action1Handler: { [weak self] in
        ...
        case .gender:
            ...
            sheet.messageColor = DemoUI.color_353535
            let maleAction = SPAlertAction.action(withTitle: "男".innerLocalized(), style: .default) { [weak self] (action) in
                self?._viewModel.updateGender(.male) { [weak self] in
                    self?.getUserOrMemberInfo()
                }
            }
            let femaleAction = SPAlertAction.action(withTitle: "女".innerLocalized(), style: .default) { [weak self] (action) in
            ...
            maleAction.titleColor = DemoUI.color_353535
            femaleAction.titleColor = DemoUI.color_353535
            cancelAction.titleColor = DemoUI.color_0584FE
            sheet.addAction(action: maleAction)
            sheet.addAction(action: femaleAction)
