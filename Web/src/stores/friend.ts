  ...
  actions: {
    ...
    async getList() {
      if (this.listPromise) {
        ...
        return this.list as FriendUserItem[];
      }
    ...
    onFriendDeleted({ data }: { data: FriendUserItem }) {
      if (!this.list) return;
    ...
    onFriendAdded({ data }: { data: FriendUserItem }) {
      if (!this.list) return;
      this.list.push(data);
    },
    onFriendReqAdded({ data }: { data: FriendApplicationItem }) {
      if ($router.currentRoute.value.name !== "newFriend") {
        this.newFriendCount++;
      }
    },
    async getFriendInfo(userID: string): Promise<FriendUserItem | undefined> {
      const list = await this.getList();
      return list.find((item) => item.userID === userID);
    },
    onFriendInfoChanged({ data }: { data: FriendUserItem }) {
      if (!this.list) return;
      const idx = this.list.findIndex((item) => item.userID === data.userID);
      if (idx > -1) {
        this.list[idx] = data;
    ...
    async getBlackInfo(userID: string): Promise<BlackUserItem | undefined> {
      const list = await this.getBlackList();
      return list.find((item) => item.userID === userID);
    },
    onBlackAdded({ data }: { data: BlackUserItem }) {
      if (!this.blackList) return;
      this.blackList.push(data);
    },
    onBlackDeleted({ data }: { data: BlackUserItem }) {
      if (!this.blackList) return;
