<template>
	<view class="zbPopover" :style="{
		...
		<view @longpress.stop="handle('longpress')" @click.stop="handle('click')" @blur="close"
			class="zb-button-popover">
			<slot></slot>
		</view>
		<view class="zb-popover" v-show="inited" ref="zb-transition" :class="[classes,`zb-popover-${composePlacement}`]"
			:style="[mergeStyle]" @touchmove="noop">
			<view class="zb-popover-arrow" :style="[arrowStyle]" :class="[{
							'zb_popper__up':composePlacement.indexOf('bottom')===0,
							'zb_popper__arrow':composePlacement.indexOf('top')===0,
							'zb_popper__right':composePlacement.indexOf('right')===0,
							'zb_popper__left':composePlacement.indexOf('left')===0,
						}]">
			</view>
			<slot name="content">
				<view :class="[{
          'horizontal__action':actionsDirection==='horizontal'
			...
			<view class="popover__masking" @touchstart="close"></view>
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		computed: {
			...
			composePlacement() {
				return this.nPlacement ?? this.placement
			},
				...
				return new Promise((resolve) => {
					this.$nextTick(() => {
						...
						selectorQuery.boundingClientRect(async (data) => {
							...
							if (typeof(this.getPlacement) === 'function') {
								placement = await this.getPlacement({
									...data[0]
								}, {
									...data[1]
								});
								this.nPlacement = placement;
							}
